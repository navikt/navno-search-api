...
class CompatibleSearchIntegrationTest : AbstractIntegrationTest() {
    ...
    fun testSearchWithEmptyTerm() {
        val result = restTemplate.getForEntity<SearchResult>(searchUrlCompatible(EMPTY_TERM)).body!!
    ...
    fun testSearchForText() {
        val result = restTemplate.getForEntity<SearchResult>(searchUrlCompatible(TEXT_TERM)).body!!
    ...
    fun testSearchForPhrase() {
        val result = restTemplate.getForEntity<SearchResult>(searchUrlCompatible(PHRASE_TERM)).body!!
    ...
    fun testSearchWithUnderfasettFilter() {
        val result = restTemplate.getForEntity<SearchResult>(
            searchUrlCompatible(ord = TEXT_TERM, f = FASETT_INNHOLD.toInt(), uf = listOf(UNDERFASETT_INFORMASJON))
        ).body!!
    ...
    fun testSearchWithTidsperiodeFilter() {
        val result = restTemplate.getForEntity<SearchResult>(
            searchUrlCompatible(ord = TEXT_TERM, daterange = TIDSPERIODE_LAST_30_DAYS.toInt())
        ).body!!
    ...
    fun testSearchWithMissingParameter() {
        val response = restTemplate.getForEntity<ErrorResponse>(searchUrlCompatible(ord = TEXT_TERM, daterange = null))
